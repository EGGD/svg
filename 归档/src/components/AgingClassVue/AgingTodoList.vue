...
<template>
  <div class="list-main">
    ...
    <AgingDetail @hide="detailhide" v-if="agingDetail.showFlg" :agingDetail="agingDetail" />
    <el-table v-loading="loading" :data="todolist.Items" :row-class-name="tableRowClassName" border style="width: 100%" height="96%">
...
export default {
  methods: {
    handleClick (row) {
      ...
      this.$store.dispatch('getAgingClassData', {
        id: row.ID,
    ...
    getATodoList () {
      ...
      this.loading = true
      this.$store.dispatch('getATodoList', {
        start: this.start,
        limit: this.limit,
